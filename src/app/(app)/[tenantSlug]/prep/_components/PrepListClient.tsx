...
import { getPrepSummary, getPrepTasksForDate, updatePrepTaskStatus } from '@/lib/actions/prep';
...
const STATUS_META: Record<PrepTaskStatus, { label: string; badge: string }> = {
    ...
    <div className="flex flex-col gap-4">
      ...
      {summary && !loading && !error && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge className={STATUS_META.pending.badge}>ממתינות: {summary.pending}</Badge>
          <Badge className={STATUS_META.in_progress.badge}>בביצוע: {summary.inProgress}</Badge>
          <Badge className={STATUS_META.done.badge}>הושלמו: {summary.done}</Badge>
          {summary.skipped > 0 && (
            <Badge className={STATUS_META.skipped.badge}>דולגו: {summary.skipped}</Badge>
          )}
        </div>
      )}
              ...
              tasks.map((task) => (
                <TableRow key={task.id}>
                  ...
                  <TableCell>
                    <Select
                      value={task.status}
                      onValueChange={(v) => handleStatusChange(task, v as PrepTaskStatus)}
                    >
                      <SelectTrigger
                      ...
                      <TooltipProvider delayDuration={150}>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span className="block truncate text-sm">{task.notes}</span>
                          </TooltipTrigger>
                          <TooltipContent className="max-w-xs">{task.notes}</TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    ) : (
