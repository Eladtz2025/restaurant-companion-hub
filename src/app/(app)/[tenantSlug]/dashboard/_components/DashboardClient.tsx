...
import { acknowledgeAlert, getAlertRules, getKPISnapshot } from '@/lib/actions/dashboard';
import { cn } from '@/lib/utils';

import { AlertRulesSheet } from './AlertRulesSheet';

import type { Role } from '@/lib/permissions';
import type { Alert, AlertRule, AlertSeverity, KPISnapshot } from '@/lib/types';
...
function severityIcon(sev: AlertSeverity) {
  if (sev === 'critical') return <AlertTriangle className="h-5 w-5 shrink-0 text-red-600" />;
  if (sev === 'warning') return <AlertTriangle className="h-5 w-5 shrink-0 text-yellow-600" />;
  return <Info className="h-5 w-5 shrink-0 text-blue-600" />;
}

export function DashboardClient({ tenantId, userRole, userId }: Props) {
  ...
  const dateActions = (
    <div className="flex items-center gap-2">
      ...
      <div className="rounded-md border px-3 py-1.5 text-sm font-medium">{formatDateHe(date)}</div>
      <Button
        variant="outline"
        size="sm"
        onClick={() => setDate(shiftDate(date, 1))}
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div className="flex items-center gap-2">
            ...
            {alerts.length > 0 && <Badge className={countBadgeClass}>{alerts.length}</Badge>}
          </div>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-muted-foreground flex flex-col items-center gap-2 py-8">
              <BellOff className="h-10 w-10 text-green-600" />
              ...
              {alerts.map((a) => (
                <li key={a.id} className="flex items-center gap-3 rounded-md border p-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-muted-foreground text-xs">{relativeTimeHe(a.firedAt)}</p>
                  </div>
                  <IfRole userRole={userRole} roles={['owner', 'manager']}>
                    <Button variant="outline" size="sm" onClick={() => handleAck(a)}>
                      אשר
                    </Button>
                  </IfRole>
                </li>
