-- ============================================================
-- MIGRATION 1: baseline (tenants, memberships, audit, functions)
-- ============================================================
...
-- ============================================================
-- MIGRATION 2: JWT custom claims hook
-- ============================================================
CREATE OR REPLACE FUNCTION auth.custom_access_token_hook(event jsonb)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = auth, public AS $$
DECLARE
  v_user_id uuid; v_tenant_id uuid; v_role text; v_claims jsonb;
BEGIN
  v_user_id := (event ->> 'user_id')::uuid;
  SELECT m.tenant_id, m.role INTO v_tenant_id, v_role
    FROM public.memberships m WHERE m.user_id = v_user_id ORDER BY m.created_at LIMIT 1;
...
-- ============================================================
-- MIGRATION 3: membership policies
-- ============================================================
...
-- ============================================================
-- MIGRATION 4: core schema (menu_items, ingredients, recipes)
-- ============================================================
...
-- ============================================================
-- MIGRATION 5 & 6: ingredient columns
-- ============================================================
ALTER TABLE public.ingredients ADD COLUMN IF NOT EXISTS pkg_qty NUMERIC CHECK (pkg_qty IS NULL OR pkg_qty > 0);
ALTER TABLE public.ingredients ADD COLUMN IF NOT EXISTS category TEXT NOT NULL DEFAULT 'other'
  CHECK (category IN ('produce', 'meat', 'fish', 'dairy', 'dry', 'alcohol', 'other'));

-- ============================================================
-- SEED: create your tenant + membership
-- (SQL Editor runs as superuser — bypasses RLS safely)
-- ============================================================
DO $$
DECLARE
  v_user_id  UUID;
...
BEGIN
  ...
  -- Insert tenant (skip if slug already exists)
  ...
  IF v_tenant.id IS NULL THEN
    SELECT * INTO v_tenant FROM public.tenants WHERE slug = 'my-restaurant';
  END IF;
  ...
  RAISE NOTICE 'Done — tenant slug: %, user_id: %', v_tenant.slug, v_user_id;
END $$;
